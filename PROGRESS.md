...
- [x] namastedev.com removed from the project (2026-07-19): code seed default now `https://example.com` only (`lib/services/seed.ts`) so it never reseeds even if the dashboard `SEED_URLS` is unset; deleted the orphaned 290KB namastedev scan dump `lib/rules/defs/wd-r.json` (loaded by nothing — rule discovery globs `*.ts`); scrubbed .env.example / DEPLOY.md / DEMO-SCRIPT.md. Live purge still needs a redeploy of the new build.
- [ ] USER-SIDE still open: warm ping cron-job.org GET /api/sites every 10 min. (SEED_URLS dashboard override no longer required — code default is example.com only.)
